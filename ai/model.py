...
BOARD_LAYERS = 17
RESIDUAL_BLOCKS = 3
NET_PARA_FILE = './model.h5'
FILE_HISTORY_FILE = './log'
USE_PREVIOUS_MODEL = True

class Network:
    def __init__(self):
        # All hyperparameters used in the model
        self._board_size = BORAD_SIZE  # the size of the playing board
        self._lr = LEARNING_REATE  # learning rate of SGD (2e-3)
        self._momentum = MOMENTUM  # nesterov momentum (1e-1)
        self._l2_coef = L2_COEF  # coefficient of L2 penalty (1e-4)
        self._mini_batch_size = MINI_BATCH_SIZE  # the size of batch when training the network
        self._fit_epochs = FIT_EPOCHS  # the number of iteration

        # Define Network
        self._build_network()

        # the following three lines are for a special bug, see also: https://www.jianshu.com/p/c84ae0527a3f
        temp_board = tr.array([[0 for _ in range(BORAD_SIZE)] for _ in range(BORAD_SIZE)])
        ...
        self._net_para_file = NET_PARA_FILE
        self._fit_history_file = FILE_HISTORY_FILE

        # Whether we use previous model or not
        self._use_previous_model = USE_PREVIOUS_MODEL
    ...
    def _build_network(self):
        # Input_Layer
        init_x = Input((BOARD_LAYERS, self._board_size, self._board_size))  # the input is a tensor with the shape 3*(15*15)
        x = init_x
    ...
    def train(self, board_list, color_list, last_move_list, pi_list, z_list):
        ...
        # Regularize Data
        board_list = tr.array([board2tensor(board_list[i], color_list[i], last_move_list[i], reshape_flag=False)
                               for i in range(len(board_list))])
        pi_list = tr.array(pi_list)
        z_list = tr.array(z_list)
...
def board2tensor(board, color, last_move, reshape_flag=True):
    # Current-Stone Layer
    cur = tr.array(tr.array(board) == color, dtype=tr.int)

    # Enemy-Stone Layer
    e = tr.array(tr.array(board) == -color, dtype=tr.int)

    # Last Step Layer
    l = tr.zeros((board.shape[0], board.shape[1]))
    if last_move is not None:
        l[last_move[0]][last_move[1]] = 1

    # Color Layer
    # flag = (1 if color == BLACK else 0)
    # c = flag * tr.ones((board.shape[0], board.shape[1]))

    # Stack cur,e,c into tensor
    tensor = tr.array([cur, e, l])
    if reshape_flag:
...
def input_encode(vec, num, size):
    mat = tr.view(vec, (size, size))  # reshape vector into matrix
    mat = board_transform(mat, num, flag=1)
    vec = tr.view(mat, (1, size ** 2))
    return vec[0]
...
def coordinate_transform(move, type, size, flag):
    if move is None:
        return None
    board = tr.zeros((size, size))
    board[move[0]][move[1]] = 1
    board_t = board_transform(board, type, flag)
    temp = tr.where(board_t == 1)
...
def board_transform(mat, num, flag=0):
    ...
    def S(mat):
        mat = R0(tr.fliplr(mat))
        return mat

    def SR1(mat):
        mat = R1(tr.fliplr(mat))
        return mat

    def SR2(mat):
        mat = R2(tr.fliplr(mat))
        return mat

    def SR3(mat):
        mat = R3(tr.fliplr(mat))
        return mat

    # Random Transformation
    if flag == 0:
        num = int(tr.random.randint(8, size=1))
